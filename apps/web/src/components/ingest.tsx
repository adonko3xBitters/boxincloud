    ...
    <div className="fixed inset-0 z-[60] grid place-items-center bg-[var(--overlay)] p-4">
      ...
      >
        <header className="flex items-center justify-between border-b border-border px-4 py-3">
          <h2 className="text-title font-semibold text-fg">Ajouter du contenu</h2>
          <button
            onClick={onClose}
            aria-label="Fermer"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          {librariesLoading ? (
            <p className="py-8 text-center text-ui text-muted">Chargement…</p>
          ) : libraries.length === 0 ? (
              ...
              */}
              ...
              >
                ...
                <p className="mt-2 text-ui font-medium text-fg">
                  Glissez vos albums ici, ou cliquez pour choisir
                </p>
                <p className="mt-1 text-meta text-subtle">
                  CBZ, CBR, PDF — et leurs équivalents ZIP et RAR
                </p>
              </div>
        ...
        {libraries.length > 0 && (
          <footer className="flex items-center gap-2 border-t border-border px-4 py-3">
            ...
            >
              {busy ? "Envoi…" : `Envoyer${waiting > 0 ? ` (${waiting})` : ""}`}
            </button>
          </footer>
        )}
      </div>
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="flex flex-col gap-1">
        <span className="text-micro uppercase tracking-wide text-subtle">Bibliothèque</span>
        <select
          value={target}
      ...
      <label className="flex flex-col gap-1">
        <span className="text-micro uppercase tracking-wide text-subtle">
          Dossier <span className="normal-case tracking-normal">(optionnel)</span>
        </span>
        <input
          value={folder}
      ...
      <div className="mb-1.5 flex items-center justify-between">
        ...
        {done > 0 && (
          <button onClick={onClearDone} className="text-meta text-accent-text hover:underline">
            retirer les {done} envoyé{done > 1 ? "s" : ""}
          </button>
        )}
      </div>
...
function StatusMark({ status }: { status: QueueItem["status"] }) {
  if (status === "done") {
    return (
      <svg viewBox="0 0 16 16" fill="currentColor" className="size-4 shrink-0 text-success" aria-label="Envoyé">
  ...
  if (status === "failed") {
    return (
      <svg viewBox="0 0 16 16" fill="none" className="size-4 shrink-0 text-danger" aria-label="Échec">
  ...
  if (status === "sending") {
    return <span className="size-2 shrink-0 animate-pulse rounded-full bg-accent" aria-label="En cours" />;
  }
  return <span className="size-2 shrink-0 rounded-full bg-border" aria-label="En attente" />;
}
...
function FirstLibrary({ onCreated }: { onCreated: () => void }) {
  ...
  const [name, setName] = useState("Ma bibliothèque");
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-4">
      <div>
        <h3 className="text-ui font-semibold text-fg">Première bibliothèque</h3>
        <p className="mt-1 text-meta leading-relaxed text-muted">
          Indiquez où vos albums sont stockés. Le stockage est vérifié avant
          d&apos;être enregistré — un chemin ou des identifiants erronés sont
          signalés tout de suite, pas au premier scan.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-1">
        <span className="text-micro uppercase tracking-wide text-subtle">Type de stockage</span>
        <div className="flex gap-1.5">
          {(["local", "s3"] as const).map((option) => (
            ...
            >
              {option === "local" ? "Dossier local" : "S3 / MinIO"}
            </button>
          ))}
        </div>
      </div>
      ...
      <label className="flex flex-col gap-1">
        <span className="text-micro uppercase tracking-wide text-subtle">
          Sous-dossier <span className="normal-case tracking-normal">(optionnel)</span>
        </span>
        <input
          value={prefix}
      ...
      <button type="submit" disabled={busy} className={buttonClass("primary", "md")}>
        {busy ? "Vérification du stockage…" : "Créer la bibliothèque"}
      </button>
    </form>
  );
}
...
function ScanButton({ libraryId }: { libraryId: string }) {
  ...
  return (
    <button
      ...
      title="Rechercher les fichiers ajoutés en dehors de boxincloud"
      className={cx(buttonClass("secondary", "sm"), "disabled:opacity-60")}
    >
      {state === "queued" ? "Parcours lancé" : state === "failed" ? "Échec" : "Analyser le stockage"}
    </button>
  );
}
...
export function AddContentButton() {
  ...
  return (
    ...
    >
      ...
      <span className="hidden sm:inline">{uploading ? "Envoi…" : "Ajouter"}</span>
    </button>
  );
}
      ...
      {active && (
        <div className="pointer-events-none fixed inset-0 z-[70] grid place-items-center bg-accent/15 backdrop-blur-[2px]">
          <div className="rounded-xl border-2 border-dashed border-accent bg-surface/90 px-8 py-6 text-center shadow-2xl">
            <UploadIcon />
            <p className="mt-2 text-title font-semibold text-fg">Déposez pour ajouter</p>
